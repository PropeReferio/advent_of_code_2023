...
def compare_two_lines_for_part_numbers(
    possible_part_nums: List[PartNumWithCoords],
    symbols: List[int],
    seen_nums: Set,
    part_numbers_total: int,
):
...
def sum_all_gear_ratios(input_lines: List[str], gear_ratios: List[int]):
    ...
    for y, line in enumerate(input_lines):
        ...
        for x, char in enumerate(line + "."):
            if char == ".":
                cur_num = add_possible_part_num_and_clear_string(
                    cur_num, possible_gear_contacts, x, y
                )
            elif char.isdigit():
                cur_num += char
            elif char == "*":
...
def sum_all_part_numbers(input_lines: List[str], part_numbers_total: int) -> Dict:
    ...
    for y, line in enumerate(input_lines):
        ...
        for x, char in enumerate(line + "."):
            if char == ".":
                cur_num = add_possible_part_num_and_clear_string(
                    cur_num, possible_part_nums, x, y
                )
            elif char.isdigit():
                cur_num += char
            else:
        ...
        part_numbers_total = compare_two_lines_for_part_numbers(
            possible_part_nums, symbols, seen_nums, part_numbers_total
        )

        # Compare part nums from previous line with current line's symbols
        part_numbers_total = compare_two_lines_for_part_numbers(
            previous_possible_part_nums, symbols, seen_nums, part_numbers_total
        )

        # Compare part nums from current line with previous line's symbols
        part_numbers_total = compare_two_lines_for_part_numbers(
            possible_part_nums, previous_symbols, seen_nums, part_numbers_total
        )
